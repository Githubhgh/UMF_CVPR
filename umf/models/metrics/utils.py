...
def calculate_activation_statistics(activations, scale=emb_scale):
    """
    Params:
    ...
    """
    activations = activations.cpu().numpy() * scale
    mu = np.mean(activations, axis=0)
    sigma = np.cov(activations, rowvar=False)
    return mu, sigma


def calculate_activation_statistics_np(activations, scale=emb_scale):
    """
    Params:
    -- activation: num_samples x dim_feat
    Returns:
...
def calculate_diversity_np(activation, diversity_times):
    ...
    dist = scipy.linalg.norm(
        (activation[first_indices] - activation[second_indices]) / 2, axis=1
    )
    return dist.mean()
