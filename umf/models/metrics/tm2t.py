...
class TM2TMetrics(Metric):
    ...
    def __init__(
        self, top_k=3, R_size=96, diversity_times=300, dist_sync_on_step=True, **kwargs
    ):
        ...
        # self.add_state("text_list", default=[], dist_reduce_fx=None)
        self.add_state("gen_motion_list", default=[], dist_reduce_fx=None)
    ...
    def compute(self, sanity_flag):
        ...
        if sanity_flag or count_seq < 900:
            return metrics
        ...
        # all_texts_ano = np.concatenate(self.text_list, axis=0)[shuffle_idx]
        all_gen_motion = torch.cat(self.gen_motion_list, axis=0).cpu()[shuffle_idx, :]
        ...
        # print('Now we are in ', os.environ.get('umf_current_epoch'))

        mu, cov = calculate_activation_statistics_np(all_genmotions)
        # gt_mu, gt_cov = calculate_activation_statistics_np(all_gtmotions)
        gt_mu, gt_cov = calculate_activation_statistics_np(all_gtmotions)
        metrics["FID"] = calculate_frechet_distance_np(gt_mu, gt_cov, mu, cov)
        os.environ["mld_fid"] = str(metrics["FID"].item())
    ...
    def update(
        ...
        # gen_motion_flat = torch.flatten(gen_motion,
        # start_dim=1).detach()
        ...
        self.gen_motion_list.append(gen_motion)


from tqdm import tqdm


def find_max_and_min_error_indices(A, B, num_indices=2):
    # Find index lists of sub-arrays with the largest and smallest errors.
    # Determine array size
    n = A.shape[0]

    # Compute Euclidean distance between each pair of sub-arrays
    errors = np.zeros((n, n))
    ...
    # Find indices of sub-arrays with the largest errors
    flat_errors = np.diagonal(errors)
    max_error_indices = np.argsort(flat_errors)[-num_indices:]

    # Find indices of sub-arrays with the smallest errors
    min_error_indices = np.argsort(flat_errors)[:num_indices]

    return max_error_indices, min_error_indices
