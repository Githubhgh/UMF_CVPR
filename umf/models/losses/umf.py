...
from .inter_loss import GeometricLoss, InterLoss
class UMFLosses(Metric):
    """
    UMF Loss
    """
    ...
    def update(self, rs_set):
        total: float = 0.0
        self.interloss_manager = InterLoss("l2", 22)
        self.loss_a_manager = GeometricLoss("l2", 22, "A")
        self.loss_b_manager = GeometricLoss("l2", 22, "B")

        # Compute the losses
        # Compute instance loss
        if self.stage in ["vae", "vae_diffusion"]:

            #prediction = rs_set['joints_rst'] # [bn, len, 44, 3]
            #target = rs_set['joints_ref']
            #mask = rs_set['mask']
            #timestep_mask = torch.ones_like(prediction[...,0,0,0])

            total += self._update_loss("recons_feature", rs_set['m_rst'],
                                       rs_set['m_ref']) # 0.0833
            # total += self._update_loss("recons_joints", rs_set['joints_rst'][...,:22,:],
            #                            rs_set['joints_ref'][...,:22,:]) # 0.1446
            # total += self._update_loss("recons_joints", rs_set['joints_rst'][...,22:,:],
            #                            rs_set['joints_ref'][...,22:,:]) # 0.1446
            total += self._update_loss("kl_motion", rs_set['dist_m'], rs_set['dist_ref']) # 8e-5
