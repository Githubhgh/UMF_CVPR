...
class InterLoss(nn.Module):
    ...
    def forward(self, pred_g_joints, tgt_g_joints, mask, time_mask):
        ...
        B, T = pred_g_joints.shape[:2]
        self.pred_g_joints = pred_g_joints.reshape(B, T, -1, self.nb_joints, 3)
        self.tgt_g_joints = tgt_g_joints.reshape(B, T, -1, self.nb_joints, 3)

        self.mask = mask
        self.timestep_mask = time_mask
    ...
    def forward_relatvie_rot(self):
        ...
        across = self.pred_g_joints[..., r_hip, :] - self.pred_g_joints[..., l_hip, :] # [32, 300, 2, 3]
        across = across / across.norm(dim=-1, keepdim=True)
        across_gt = self.tgt_g_joints[..., r_hip, :] - self.tgt_g_joints[..., l_hip, :] # [32, 300, 2, 3]
        across_gt = across_gt / (across_gt.norm(dim=-1, keepdim=True) + 1e-5) ## Using custom mean/std may produce NaN

        y_axis = torch.zeros_like(across)
        y_axis[..., 1] = 1

        forward = torch.cross(y_axis, across, axis=-1)
        forward = forward / (forward.norm(dim=-1, keepdim=True) + 1e-5)
        forward_gt = torch.cross(y_axis, across_gt, axis=-1) # [32, 300, 2, 3]
        forward_gt = forward_gt / (forward_gt.norm(dim=-1, keepdim=True) + 1e-5)
        ...
        if torch.isnan(self.losses["RO"]).item():
            #self.losses["RO"] = torch.tensor(1e-5, device=self.losses["RO"].device)
            self.losses["RO"] = torch.tensor(5e-3, device=self.losses["RO"].device)
...
class GeometricLoss(nn.Module):
    ...
    def forward(self, pred_g_joints, tgt_g_joints, mask, time_mask):
        ...
        B, T = pred_g_joints.shape[:2]
        self.pred_g_joints = pred_g_joints.reshape(B, T, self.nb_joints, 3)
        self.tgt_g_joints = tgt_g_joints.reshape(B, T, self.nb_joints, 3)
        self.mask = mask
        self.timestep_mask = time_mask
