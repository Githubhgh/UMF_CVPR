...
def _is_unset_value(value):
    ...
    return text in sentinel_values


def _override_if_set(target, key, value):
    if _is_unset_value(value):
        return
    target[key] = _to_str(value)


def _apply_umf_base_overrides(cfg):
    paths = getattr(cfg, "PATHS", None)
    if paths is None:
        return cfg

    # Core roots and assets
    _override_if_set(cfg, "FOLDER", getattr(paths, "EXPERIMENT_FOLDER", None))
    if hasattr(cfg, "TEST"):
        _override_if_set(cfg.TEST, "FOLDER", getattr(paths, "RESULT_FOLDER", None))

    if hasattr(cfg, "DATASET"):
        hm_root_raw = getattr(paths, "HUMANML3D_ROOT", None)
        hm_root = _to_str(hm_root_raw)
        if not _is_unset_value(hm_root_raw) and hasattr(cfg.DATASET, "HUMANML3D"):
            cfg.DATASET.HUMANML3D.ROOT = hm_root
            cfg.DATASET.HUMANML3D.SPLIT_ROOT = hm_root
        _override_if_set(
            cfg.DATASET,
            "WORD_VERTILIZER_PATH",
            getattr(paths, "WORD_VECTORIZER_ROOT", None),
        )

    if hasattr(cfg, "model"):
        _override_if_set(cfg.model, "t2m_path", getattr(paths, "T2M_DEPS_ROOT", None))

    # Optional single-file checkpoint overrides
    if hasattr(cfg, "TRAIN"):
        _override_if_set(cfg.TRAIN, "PRETRAINED_VAE", getattr(paths, "PRETRAINED_VAE", None))
        _override_if_set(cfg.TRAIN, "PRETRAINED_INDI", getattr(paths, "PRETRAINED_INDI", None))
        _override_if_set(cfg.TRAIN, "PRETRAINED_REAC", getattr(paths, "PRETRAINED_REAC", None))
        _override_if_set(cfg.TRAIN, "PRETRAINED", getattr(paths, "PRETRAINED", None))
    if hasattr(cfg, "TEST"):
        _override_if_set(cfg.TEST, "CHECKPOINTS", getattr(paths, "TEST_CHECKPOINTS", None))
...
def instantiate_from_config(config):
    ...
    source_file = inspect.getfile(get_obj_from_str(config["target"]))
    print(f"Target object is defined in: {source_file}")
...
def parse_args(phase="train"):
    ...
    if phase in ["train", "test", "demo"]:
        ...
        group.add_argument(
            ...
            type=str,
            required=False,
            default="./configs/umf_base.yaml",
            help="optional base config for centralized runtime paths",
        )
    ...
    cfg_umf_base = _load_optional_yaml(getattr(params, "cfg_umf_base", None))
    cfg = OmegaConf.merge(cfg_exp, cfg_model, cfg_assets, cfg_umf_base)
    cfg = _apply_umf_base_overrides(cfg)
