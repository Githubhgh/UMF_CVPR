...
from .utils import *
from .Interhuman import MergedDataModule
...
def get_collate_fn(name, phase="train"):
    if name.lower() in ["humanml3d", "kit"]:
        return mld_collate
    elif name.lower() in ["interhuman"]:
        return ih_collate
    raise NotImplementedError(f"Unsupported dataset for this branch: {name}")


# map config name to module&path
dataset_module_map = {
    "humanml3d": HumanML3DDataModule,
    "interhuman": MergedDataModule,
}
...
def get_datasets(cfg, logger=None, phase="train"):
    ...
    for dataset_name in dataset_names:
        if dataset_name.lower() in ["humanml3d", "kit"]:
        ...
        elif dataset_name.lower() in ["interhuman"]:
            collate_fn = get_collate_fn(dataset_name, phase)
            dataset = dataset_module_map[dataset_name.lower()](cfg=cfg, 
                                                               batch_size=cfg.TRAIN.BATCH_SIZE, 
                                                               num_workers=cfg.TRAIN.NUM_WORKERS,
                                                               collate_fn=collate_fn,)
            datasets.append(dataset)
            ...
            raise NotImplementedError(
                f"Unsupported dataset in this branch: {dataset_name}"
            )
