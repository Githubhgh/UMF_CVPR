...
from umf.data.humanml.scripts.motion_process import (qrot,
...
from umf.data.humanml.utils.word_vectorizer import WordVectorizer
...
class Text2MotionDatasetV2(data.Dataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        default_data_root = pjoin(repo_root, "data", "HUMANML3D")
        default_glove_root = pjoin(repo_root, "data", "glove")
        default_mean_org_path = pjoin(repo_root, "assets", "runtime", "humanml3d_meta", "mean_org.npy")
        default_std_org_path = pjoin(repo_root, "assets", "runtime", "humanml3d_meta", "std_org.npy")
        default_text_dir = pjoin(repo_root, "data", "HUMANML3D", "texts")
        default_motion_dir = pjoin(repo_root, "data", "HUMANML3D", "new_joint_vecs")

        data_root = kwargs.get("data_root", default_data_root)
        self.data_root = data_root
        self.mode = mode
        self._stats_split = self._resolve_split(mode)
        if mode == 'train':
        ...
        glove_root = kwargs.get("glove_root", default_glove_root)
        mean_org_path = kwargs.get("mean_org_path", default_mean_org_path)
        std_org_path = kwargs.get("std_org_path", default_std_org_path)
        text_dir = kwargs.get("text_dir", default_text_dir)
        motion_dir = kwargs.get("motion_dir", default_motion_dir)

        w_vectorizer = WordVectorizer(glove_root, "our_vab")
        mean_org = np.load(mean_org_path)
        std_org = np.load(std_org_path)
        mean, std = self._load_ih_stats_or_none(self._stats_split)
        ...
        # else:
        #     maxdata = 1e10
        #maxdata = 1000 if self.mode == 'train' else 99999
        maxdata = 1e10
        if 'debug' in os.environ.get('UMF') and mode=='train':
            maxdata = 1200
        ...
        if mean is None or std is None:
            mean, std = self._compute_ih_stats_from_loaded_data(data_dict)
            self._save_ih_stats(self._stats_split, mean, std)

        self.mean = mean
        self.std = std
        self.mean_org = mean_org
        self.std_org = std_org
        self.length_arr = np.array(length_list)
        self.data_dict = data_dict
    ...
    def _resolve_split(self, mode):
        return "train" if mode == "train" else "test"

    def _load_ih_stats_or_none(self, split):
        candidates = [
            (
                pjoin(self.data_root, f"Mean_ihmix266_{split}.npy"),
                pjoin(self.data_root, f"Std_ihmix266_{split}.npy"),
            ),
            (
                pjoin(self.data_root, "Mean_ihmix266.npy"),
                pjoin(self.data_root, "Std_ihmix266.npy"),
            ),
        ]
        for mean_path, std_path in candidates:
            ...
            if mean.shape != (266,) or std.shape != (266,):
                raise RuntimeError(
                    f"Invalid ihmix266 stats shape for split={split}: "
                    f"mean={mean.shape}, std={std.shape}, paths=({mean_path}, {std_path})"
                )
            if not (np.isfinite(mean).all() and np.isfinite(std).all()):
                raise RuntimeError(
                    f"Non-finite ihmix266 stats for split={split} from paths=({mean_path}, {std_path})"
                )
            return mean, std
        return None, None

    def _compute_ih_stats_from_loaded_data(self, data_dict):
        if not data_dict:
            raise RuntimeError(
                f"Cannot compute ihmix266 stats for split={self._stats_split}: "
                f"no loaded samples under {self.data_root}"
            )
        ...
        for sample_name, sample in data_dict.items():
            ...
            if motion.ndim != 2 or motion.shape[1] != 263:
                raise RuntimeError(
                    f"Invalid motion shape while computing ihmix266 stats: "
                    f"name={sample_name}, shape={motion.shape}"
                )
            hm_motion = motion.astype(np.float32, copy=False)
            _, r_pos = recover_root_rot_pos(torch.from_numpy(hm_motion).float())
            ih_motion = np.concatenate([hm_motion, np.asarray(r_pos, dtype=np.float32)], axis=1)
            if ih_motion.shape[1] != 266:
                raise RuntimeError(
                    f"Unexpected ih motion shape while computing ihmix266 stats: "
                    f"name={sample_name}, shape={ih_motion.shape}"
                )
            if not np.isfinite(ih_motion).all():
                raise RuntimeError(
                    f"Non-finite ih motion while computing ihmix266 stats: name={sample_name}"
                )

            ih_motion = ih_motion.astype(np.float64, copy=False)
            sample_sum = ih_motion.sum(axis=0)
            sample_sumsq = np.square(ih_motion).sum(axis=0)
            if sum_vec is None:
                sum_vec = sample_sum
                sumsq_vec = sample_sumsq
            else:
                sum_vec += sample_sum
                sumsq_vec += sample_sumsq
            total_frames += ih_motion.shape[0]

        if total_frames == 0:
            raise RuntimeError(
                f"Cannot compute ihmix266 stats for split={self._stats_split}: "
                f"loaded samples contained zero valid frames under {self.data_root}"
            )
        ...
        if not (np.isfinite(mean).all() and np.isfinite(std).all()):
            raise RuntimeError(
                f"Non-finite ihmix266 stats computed for split={self._stats_split}"
            )
        return mean.astype(np.float32), std.astype(np.float32)

    def _save_ih_stats(self, split, mean, std):
        mean_path = pjoin(self.data_root, f"Mean_ihmix266_{split}.npy")
        std_path = pjoin(self.data_root, f"Std_ihmix266_{split}.npy")
    ...
    def __getitem__(self, item):
        ...
        hm_motion = np.array(motion)
        r_rot_quat, r_pos = recover_root_rot_pos(torch.from_numpy(hm_motion).float())
        ih_motion = np.concatenate([hm_motion, np.array(r_pos)], axis=1)
        motion = ih_motion
        ...
        motion = motion[idx:idx + m_length]
        "Z Normalization"
        #motion[..., :263] = (motion[..., :263] - self.mean_org) / self.std_org
        motion = (motion - self.mean) / self.std
