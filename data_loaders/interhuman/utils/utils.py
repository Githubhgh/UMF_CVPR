...
from data_loaders.interhuman.common.quaternion import *

from data_loaders.interhuman.utils.rotation_conversions import *
...
def rigid_transform(relative, data): # [3, ] # [len, 262]

    global_positions = data[..., :22 * 3].reshape(data.shape[:-1] + (22, 3)) # [len, 22, 3]
    global_vel = data[..., 22 * 3:22 * 6].reshape(data.shape[:-1] + (22, 3)) # [len, 22, 3]

    relative_rot = relative[0]
    relative_t = relative[1:3]
    relative_r_rot_quat = np.zeros(global_positions.shape[:-1] + (4,)) # (len, 22, 4)
    relative_r_rot_quat[..., 0] = np.cos(relative_rot)
    relative_r_rot_quat[..., 2] = np.sin(relative_rot)
    global_positions = qrot_np(qinv_np(relative_r_rot_quat), global_positions) # qinv_np(relative_r_rot_quat): [len,22,4] global_positions (len, 22, 3)
    global_positions[..., [0, 2]] += relative_t
    data[..., :22 * 3] = global_positions.reshape(data.shape[:-1] + (-1,)) # [len, 66]
...
def cal_rela(motion1, motion2):
    # Compute relative positions for all node pairs
    relative_positions = S1 - S2  # shape: (T, N, 3)

    # Compute pairwise distances (for each time step)
    distances = np.linalg.norm(relative_positions, axis=2)  # shape: (T, N)

    # Compute unit direction vectors
    norms = np.linalg.norm(relative_positions, axis=2, keepdims=True)  # shape: (T, N, 1)
    unit_directions = relative_positions / norms  # shape: (T, N, 3)

    # Compute average distance for each node pair
    average_distances = np.mean(distances, axis=0)  # shape: (N, N)

    # Compute average direction for each node pair
    # Average unit directions across all time steps
    average_unit_directions = np.mean(unit_directions, axis=0)  # shape: (N, N, 3)

    # Print results
    print("Average distance matrix for all node pairs (N x N):")
    print(average_distances)

    print("\nAverage unit direction vectors for all node pairs (N x N x 3):")
    print(average_unit_directions)
    return distances, unit_directions

def rigid_transform_th(relative, data):

    global_positions = data[..., :22 * 3].reshape(data.shape[:-1] + (22, 3)) # [96,300,22,3]
    global_vel = data[..., 22 * 3:22 * 6].reshape(data.shape[:-1] + (22, 3)) # [96,300,22,3]

    relative = relative.unsqueeze(-2)
    relative =  torch.tile(relative, (1, 22, 1))
    relative_rot = relative[..., 0]
    relative_t = relative[..., 1:3]
    relative_r_rot_quat = torch.zeros(global_positions.shape[:-1] + (4,)).to(relative.device) # [96,300,22,4]
    relative_r_rot_quat[..., 0] = torch.tile(torch.cos(relative_rot), [1,1,1])
    relative_r_rot_quat[..., 2] = torch.sin(relative_rot)
    global_positions = qrot(qinv(relative_r_rot_quat), global_positions)
    global_positions[..., [0, 2]] += relative_t
    data[..., :22 * 3] = global_positions.reshape(data.shape[:-1] + (-1,))
    global_vel = qrot(qinv(relative_r_rot_quat), global_vel)
    data[..., 22 * 3:22 * 6] = global_vel.reshape(data.shape[:-1] + (-1,))
...
class MotionNormalizer():
    def __init__(self):
        mean = np.load("data/InterHuman/global_mean.npy")
        std = np.load("data/InterHuman/global_std.npy")
...
def process_motion_np(motion, feet_thre, prev_frames, n_joints):
    ...
    # motion: [155, 192]
    positions = motion[:, :n_joints*3].reshape(-1, n_joints, 3) # [155,22,3]
    rotations = motion[:, n_joints*3:] # [155, 126]
    ...
    '''XZ at origin'''
    root_pos_init = positions[prev_frames] # [22,3]
    ...
    #
    feet_l, feet_r = foot_detect(positions, feet_thre) # [154,2], [154,2]
    ...
    data = joint_positions[:-1] # joint_positions[:-1]: [154, 22*3=66]
    data = np.concatenate([data, joint_vels], axis=-1) # joint_vels: [154, 22*3=66]
    data = np.concatenate([data, rot_data[:-1]], axis=-1) # rot_data[:-1]: [154, 21*6=126]
    data = np.concatenate([data, feet_l, feet_r], axis=-1) # feet_l&feet_r: [154, 2] & [154, 2]

    return data, root_quat_init, root_pose_init_xz[None] # Non-canonical representation -> data: [154, 66+66+126+4=262]
