...
class InterLoss(nn.Module):
    def __init__(self, recons_loss, nb_joints, mode='idm', define_weights = None):
        ...
        self.weights = {}
        self.weights["RO"] = 0.01 # 0.01
        self.weights["JA"] = 3
        self.weights["DM"] = 3

        if define_weights is not None:
            #print(define_weights)
            ...
            def extract_numbers(s):
                ...
                return numbers
            weights__ = extract_numbers(define_weights)
            self.weights["RO"] = weights__[0]
            self.weights["JA"] = weights__[1]
            self.weights["DM"] = weights__[2]
    ...
    def forward(self, motion_pred, motion_gt, mask, timestep_mask): 
        # mask: related to the sequence length 
        # timestep_mask: related to the timestep bar limitation
        ...
        self.pred_g_joints = prediction[..., :self.nb_joints * 3].reshape(B, T, -1, self.nb_joints, 3) # self.nb_joints:22
        self.tgt_g_joints = target[..., :self.nb_joints * 3].reshape(B, T, -1, self.nb_joints, 3)

        self.mask = mask
        self.timestep_mask = timestep_mask

        self.forward_distance_map(thresh=1) #DM loss -> distance map loss
        self.forward_joint_affinity(thresh=0.1)
        self.forward_relatvie_rot() #relative orientation loss
        self.accum_loss()
    ...
    def forward_distance_map(self, thresh):
        pred_g_joints = self.pred_g_joints.reshape(self.mask.shape[:-1] + (-1,)) # [32, 300, 2, 22, 3] -> [32, 300, 2, 66]
        tgt_g_joints = self.tgt_g_joints.reshape(self.mask.shape[:-1] + (-1,))

        pred_g_joints1 = pred_g_joints[..., 0:1, :].reshape(-1, self.nb_joints, 3) # person 1 joints prediction -> [9600, 22, 3]
        pred_g_joints2 = pred_g_joints[..., 1:2, :].reshape(-1, self.nb_joints, 3) # person 2 joints prediction
        tgt_g_joints1 = tgt_g_joints[..., 0:1, :].reshape(-1, self.nb_joints, 3) # person 1 joints ground truth
        tgt_g_joints2 = tgt_g_joints[..., 1:2, :].reshape(-1, self.nb_joints, 3) # person 2 joints ground truth
    ...
    def accum_loss(self):
        loss = 0
        for term in self.losses.keys(): # ['simple', 'DM', 'JA', 'RO']
...
class GeometricLoss(nn.Module):
    def __init__(self, recons_loss, nb_joints, name, mode='idm'):
    ...
    # calucalating the pose is no need cause the input have included such information
    ...
    def forward_contact(self):
        ...
        feet_vel = self.pred_g_joints[:, 1:, self.fids, :] - self.pred_g_joints[:, :-1, self.fids,:] # self.fids [7,10,8,11] feet and keen; left and right
    ...
    def forward_bone_length(self):
        ...
        pred_bones = []
        tgt_bones = []
        for chain in kinematic_chain: # [0, 2, 5, 8, 11] leg, [0, 1, 4, 7, 10] leg, [0, 3, 6, 9, 12, 15] torso, [9, 14, 17, 19, 21] hand, [9, 13, 16, 18, 20] hand
        ...
        pred_bones = torch.cat(pred_bones, dim=-1) # 4 bones * 5 chains
    ...
    def accum_loss(self):
        loss = 0
        for term in self.losses.keys(): # ['VEL_A', 'BL_A', 'FC_A']
            loss += self.losses[term]
    ...
    def foot_detect(self, feet_vel, feet_h, thres):
        ...
        # velfactor [0.0010, 0.0010, 0.0010, 0.0010]  
        # heightfactor [0.12, 0.05, 0.12, 0.05]
    ...
    def p2p_zero_loss(self):
        loss = 0
        for term in self.losses.keys():
            self.losses[term] = torch.zero_(self.losses[term])
        self.losses[self.name] = torch.zero_(self.losses[term])