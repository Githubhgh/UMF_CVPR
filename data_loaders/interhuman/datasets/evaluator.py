...
from data_loaders.interhuman.datasets import InterHumanDataset
from data_loaders.interhuman.models import *
import copy
from data_loaders.interhuman.datasets.evaluator_models import InterCLIP
...
def build_models(cfg):
    ...
    checkpoint = torch.load(pjoin('eval_model/interclip.ckpt'), weights_only=False, map_location="cpu")
...
from data_loaders.interhuman.interhuman import get_config
class EvaluatorModelWrapper(object):

    def __init__(self, cfg, device=None):
        if isinstance(cfg, str) and os.path.exists(cfg):
            cfg = get_config(cfg)
        elif not hasattr(cfg, "clone"):
            fallback_cfgs = [
                "configs/eval_model.yaml",
                "data_loaders/interhuman/configs/eval_model.yaml",
            ]
            cfg_path = next((path for path in fallback_cfgs if os.path.exists(path)), None)
            if cfg_path is None:
                raise FileNotFoundError(
                    "InterHuman evaluator config not found. "
                    "Checked: configs/eval_model.yaml and data_loaders/interhuman/configs/eval_model.yaml"
                )
    ...
    def get_co_embeddings_524(self, motions, text, motion_lens):
        self.device = motions.device
        self.model = self.model.to(self.device)
        
        with torch.no_grad():
            motions = motions.detach().to(self.device).float()
            motion_lens = np.array(motion_lens)
            align_idx = np.argsort(motion_lens)[::-1].copy()
            motions = motions[align_idx]
            motion_lens = motion_lens[align_idx]
            text = list(text)
    ...
    def get_motion_embeddings_524(self, motions, text, motion_lens):
        self.device = motions.device
        self.model = self.model.to(self.device)
        with torch.no_grad():
            motions = motions.detach().to(self.device).float()
