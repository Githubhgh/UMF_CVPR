...
def main():
    ...
    missing = [key for key in model_state_dict if key not in state_dict]
    unexpect = [key for key in state_dict if key not in model_state_dict]
    if missing:
        logger.info(f"Keys initialised from scratch (new layers): {missing}")
    if unexpect:
        logger.info(f"Keys in checkpoint not in model (ignored): {unexpect}")

    model.load_state_dict(new_state_dict, strict=False)
    if hasattr(model, "init_flow_schedulers"):
        model.init_flow_schedulers()
        logger.info("Flow schedulers initialized after checkpoint loading.")
