    ...
    def forward(self,
        ...
        if transl is not None:
            ...
            # joints_from_verts_h36m = joints_from_verts_h36m - joints_from_verts_h36m[:, self.root_idx_17, :].unsqueeze(1).detach()
                ...
                joints += transl.unsqueeze(dim=0)
                vertices += transl.unsqueeze(dim=0)
                joints_from_verts_h36m += transl.unsqueeze(dim=0)
            
            #vertices = vertices - joints_from_verts_h36m[:, self.root_idx_17, 1].unsqueeze(1).detach()
            #joints = joints - joints[:, self.root_idx_smpl, 1].unsqueeze(1).detach()
            #joints_from_verts_h36m = joints_from_verts_h36m - joints_from_verts_h36m[:, self.root_idx_17, 1].unsqueeze(1).detach()
